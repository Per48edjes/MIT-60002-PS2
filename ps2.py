...
# represented?
#
# Answer: Buildings are the nodes, and the edges (which are directed) are weighted by total distances between nodes
# and total distance spent outside between nodes.
...
def load_map(map_filename):
    ...
    with open(map_filename, 'r') as f:
        for line in f:
            ...
            digraph.add_edge(WeightedEdge(src, dest, int(total_distance), int(outdoor_distance)))
    print(digraph)
    return digraph
...
#
# Answer: The objective function is the distance traversed (to be minimized) while the 
# constraint is distance spent outside (limited). 
#
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist, best_path):
    ...
    if not (digraph.has_node(Node(start)) and digraph.has_node(Node(end))):
        raise ValueError('Non-existent node in graph')
    ...
    # Base Case: starting at end == reached destination
    if start == end:
    ...
    # Recursive Case: Not at end, check all children edges
    else:

        ## Loop through every child of 'start' node
        for edge in digraph.get_edges_for_node(Node(start)):
            ...
            if str(edge.get_destination()) not in curr_path:
                ...
                # Conditions to continue exploration of child: not exceeding maximum outdoor distance constraint,
                # no best path found yet or this new path isn't longer than shortest path found so far
                # in the path.
